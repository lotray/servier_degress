...
<html>
  ...
  <body>
  	...
  	<button onClick="show()">按标题查询</button>
  	<button onClick="show1()">按发布时间范围查询</button>
  	<!-- 按标题查询 -->
  	<div id="title" >
  		<s:form action="findrewardbytitle">
    		<s:textfield name="reward.title" label="标题关键字查询"/>
    		<s:submit value="查询"/>
    	</s:form>
  	</div>
  	<!-- 按时间范围查询 -->
    <div id="time" style="display:none">
  		<s:form action="findAllbytime">
    		<s:textfield name="stime" label="开始日期" onclick="fPopCalendar(event,this,this)"/>
    		<s:textfield name="etime" label="结束日期" onclick="fPopCalendar(event,this,this)"/>
    		<s:submit value="查询"/>
    	</s:form>
  	</div>
<table border="1">
   ...
   <tr>
  	...
  	<s:iterator value="rewards" id="rewards">
   			...
   			<td><s:a action="jumpReward1">修改
   					<s:param name="reward.Rid" value="#rewards.Rid"/>
   			</s:a></td>
   			<td><s:a action="deleteReward1">删除
   					<s:param name="reward.Rid" value="#rewards.Rid"/>
   			</s:a></td>
   			</tr>
   		</s:iterator>
